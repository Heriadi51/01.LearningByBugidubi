...
class PuzzleMain3 : AppCompatActivity() {
    @RequiresApi(Build.VERSION_CODES.N)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_puzzle_main3)
        ...
        dragView3.setOnLongClickListener {
            val clipText = "Jangan Makan Berdiri"
            val item = ClipData.Item(clipText)
            val mimeTypes = arrayOf(ClipDescription.MIMETYPE_TEXT_PLAIN)
            val data = ClipData(clipText, mimeTypes, item)
    ...
    private val dragListener3 = View.OnDragListener { view, event ->
        when (event.action) {
            ...
            DragEvent.ACTION_DROP -> {
                ...
                val dragData = item.text
                textDetail.text = dragData
                MediaPlayer.create(this, R.raw.makan_berdiri).start()
